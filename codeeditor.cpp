...
void CodeEditor::paintEvent(QPaintEvent *event){
    ...
    //painter.setPen(padlike_colorgroup[]);
...
bool CodeEditor::MatchForFolding(QTextBlock currentBlock,QTextBlock &leftBlock,QTextBlock &rightBlock){         //根据当前block，寻找{}上下语句块
    ...
    while(true){
        ...
        if(data){
            ...
            for(int i =0;i<infos.size();++i){
                ParenthesisInfo *info = infos.at(i);
                if(info->character == '{' && current.blockNumber()!=currentBlock.blockNumber())         //不在同一块
                    ++count;
                if(info->character == '}'){
                    if(count == 0){
                        rightBlock = current;
                        //qDebug()<<rightBlock.blockNumber();
                        flag = true;
                        break;
                    }
                    else
...
void CodeEditor::FunctionFold(int y){
    ...
    if(MatchForFolding(blk_cur,blk_top,blk_bottom)){
        ...
        //isCursorMove = blk_cur.blockNumber()<firstVisibleBlock().blockNumber() ? true : false ;

        if(blk_top.blockNumber() != 0 && JudgeLeftBracket(blk_top.previous()))
            blk_top = blk_top.previous();

        if(isCursorMove){               //移动光标到top_block
            QTextCursor cursor = textCursor();
        ...
        if(MatchForFolding(block,topBlock,bottomBlock)&&
           topBlock.blockNumber()>=firstVisibleBlock().blockNumber()){
            ...
            if(topBlock.next().isVisible()){                //三个三角形缩成一个实心
                ...
                painter.drawPolygon(points1,3,Qt::WindingFill);
                /*QPointF points2[3] = {
                    QPointF(8,foldPosition+littleChange+5),
                    QPointF(8,foldPosition+littleChange+11),
                    QPointF(10,foldPosition+littleChange+8)
                };
                //painter.drawPolygon(points2,3);
                QPointF points3[3] = {
                    QPointF(9,foldPosition+littleChange+7),
                    QPointF(9,foldPosition+littleChange+10),
                    QPointF(9,foldPosition+littleChange+8)
                };
                //painter.drawPolygon(points3,3);*/
            }
        }
